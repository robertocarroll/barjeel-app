...
PlayModule.resumeQuiz = function() {
  ...
  PlayModule.newLocalCollection = new Barjeel.Collections.PlayCollection(JSON.parse(localStorage.getItem('barjeel-app-questions')));

  PlayModule.questions = PlayModule.newLocalCollection
    .chain()
    .filter(function(m) {
      return m.get('use')
    })
    ...
    if (PlayModule.questions.length > 0) {
     console.log('this is how many questions from resumeQuiz: ' + PlayModule.questions);
     PlayModule.getQuestion();
    }
...
// Get all the questions from the collection and filter by use value
  ...
  if (PlayModule.questions.length > 0) {
    var questions = PlayModule.questions;
    ...
    var currentQuestion = PlayModule.newLocalCollection.get(rand);
    console.log(currentQuestion);

    //set the status of that question to used
    currentQuestion.set("use", false);
    localStorage.setItem('barjeel-app-questions', JSON.stringify(PlayModule.newLocalCollection.toJSON()));
